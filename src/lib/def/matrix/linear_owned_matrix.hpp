...
namespace matrix {
    ...
    class LinearOwnedMatrix : ValueContainer_<ValueType_> {
        ...
        LinearOwnedMatrix adjacent_cal(size_t size){
            This ans (size, size);
            if (size == 1){
                ans[0][0] = 1;
            } else {
                ...
                for (size_t i = 0; i < size; ++i) {
                    for (size_t j = 0; j < size; ++j) {
                        ...
                        ans[j][i] = determinant_calculate(temp, n - 1) * ((i + j) % 2 == 1 ? -1 : 1);
                    }
                }
            }
            return ans;
        ...
        public:
            ...
            LinearOwnedMatrix convolution(LinearOwnedMatrix & rhs) const {
                ...
                if (row() < rhs.row() || col() < rhs.col()){
                    throw MatrixStructureMismatchingException("LinearOwnedMatrix convolution with bigger ")
                }
